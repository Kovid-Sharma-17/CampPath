...
| `route-editor.html` / `route-editor.js` | `/Users/kovidsharma/Documents/ChatGPT/VTHack26 2/dist/route-editor.html` | Standalone page (linked from the header) for drawing a new route on the real campus map — click a start entrance/junction, an end, then trace the path — and generating the `scripts/add_route.py` command for it. Reuses `router.mjs`'s `buildGraph` for the node list, so it can never suggest an endpoint the router wouldn't recognize. Writes nothing itself; only produces a command to review and run. |
...
| --- | --- | --- |
| `buildings.geojson` | `/Users/kovidsharma/Documents/ChatGPT/VTHack26 2/dist/data/buildings.geojson` | 462 named VT buildings. 19 are the routable pilot zone (16 supplied + New Classroom Building, Davidson Hall, Williams Hall added 2026-09-19) with full evidence fields; the other 443, added 2026-09-19 via `scripts/add_all_campus_buildings.py`, are marker-only (name + coordinate, `official` confidence from VT's Enterprise GIS) with no entrances yet — see the README's **Adding a building's entrances**. |
...
| `add_route.py` | `/Users/kovidsharma/Documents/ChatGPT/VTHack26 2/scripts/add_route.py` | CLI for appending one path segment to `paths.geojson` without hand-editing the file — auto-assigns the next segment id and defaults every accessibility field to `unknown`/`inferred`. The output of `dist/route-editor.html`'s visual tool is a call to this script. |
| `add_entrance.py` | `/Users/kovidsharma/Documents/ChatGPT/VTHack26 2/scripts/add_entrance.py` | CLI for adding one entrance to `entrances.geojson` for a building that doesn't have one yet — most of the 443 buildings `add_all_campus_buildings.py` added. Auto-assigns the next `N-<building>-E<n>` node id; defaults to `unknown`/`inferred`. |
| `add_all_campus_buildings.py` | `/Users/kovidsharma/Documents/ChatGPT/VTHack26 2/scripts/add_all_campus_buildings.py` | One-time, safely-rerunnable pass (applied 2026-09-19) that added all 443 remaining named, existing VT buildings from the Enterprise GIS inventory — including the residential side of campus, which the original pilot omitted. Markers only: no entrances, paths, or connectors. Skips names already curated in the pilot and `NON-BUILDING` records (ponds, towers). |
| `vt_gis_all_buildings_20260919.py` / `.raw.json` | `/Users/kovidsharma/Documents/ChatGPT/VTHack26 2/scripts/vt_gis_all_buildings_20260919.py` | The raw VT Enterprise GIS building inventory (583 records, `community='VIRGINIA TECH' AND status='Existing Conditions'`) that `add_all_campus_buildings.py` merges in. |
