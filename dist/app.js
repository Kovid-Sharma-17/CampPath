...
import {distanceText, arrivalText, routeNarration, currentGuidance, createCueTracker, createSpeaker} from './guidance.mjs';
...
const state={origin:'VT-GOODWIN',destination:'VT-DDS',originEntrance:null,destinationEntrance:null,avoidStairs:true,gps:null,watch:null,navigating:false,voice:true,reroutedAt:0,progress:0,follow:true};
...
function wire(){
 ...
 $('#avoid-stairs').onchange=e=>{stopNavigation();state.avoidStairs=e.target.checked;render();};
 ...
 $('#settings-form').onsubmit=e=>{e.preventDefault();sessionStorage.setItem('camp-path-gemini-key',$('#gemini-key').value.trim());localStorage.setItem('camp-path-model',$('#gemini-model').value.trim()||'gemini-2.5-flash');state.voice=$('#voice-enabled').checked;if(!state.voice)speaker.stop();updateMuteButton();localStorage.setItem('camp-path-voice',$('#voice-select').value);$('#settings-dialog').close();toast('Settings saved.');};
...
function render(fit=true){
 speaker.stop();cueTracker.reset();state.lastGuidance=null;
...
const speaker=createSpeaker(window.speechSynthesis,text=>new SpeechSynthesisUtterance(text),{onError:toast,onState:active=>{const b=$('#read-route');if(b){b.setAttribute('aria-label',active?'Stop reading directions':'Read route directions aloud');b.title=active?'Stop reading':'Read directions';b.innerHTML=icon(active?'stop':'volume');}}});
function speak(text){const voice=window.speechSynthesis?.getVoices().find(v=>v.voiceURI===localStorage.getItem('camp-path-voice'));speaker.speak(text,{voice});}
function waitingForGPS(message){speaker.stop();state.lastGuidance=null;locationMessage(message);if(state.navigating){$('#navigation-banner').hidden=false;$('#navigation-banner').innerHTML=icon('locate')+'<div><strong>Waiting for clearer GPS</strong><small>Turn guidance is paused. Check the map and your surroundings.</small></div><button id="stop-live" class="icon-button" aria-label="Stop navigation">'+icon('close')+'</button>';$('#stop-live').onclick=()=>stopNavigation();}}
...
function useLocation(navigate=false){
 ...
 state.navigating=navigate;state.follow=true;locating=!navigate;firstFix=true;offRouteSamples=0;cueTracker.reset();speaker.stop();state.lastGuidance=null;
...
function receivePosition(position){
 if(!usablePosition(position)){waitingForGPS('Waiting for a fresh GPS reading…');return;}
 ...
 if(c.accuracy>25){waitingForGPS('GPS accuracy is ±'+Math.round(c.accuracy)+' m. Waiting for a clearer signal before giving turns.');return;}
 locationMessage('Live GPS active.');
 ...
 if(decision.action==='arrive'){
  const message=arrivalText(route,{arrived:true});stopNavigation(false);locationMessage(message);$('#navigation-banner').hidden=false;$('#navigation-banner').innerHTML=icon('flag')+'<div><strong>'+ (route.endEntrance?.kind==='nearby_path'?'Mapped path reached':'Destination nearby')+'</strong><small>'+esc(message)+'</small></div><button id="stop-live" class="icon-button" aria-label="Dismiss arrival">'+icon('close')+'</button>';$('#stop-live').onclick=()=>stopNavigation();if(state.voice)speak(message);return;
 }
 if(decision.action==='reroute'&&Date.now()-state.reroutedAt>10000){state.reroutedAt=Date.now();offRouteSamples=0;state.origin='GPS';syncInputs();render(false);progress=routeProgress(route,state.gps);if(!route.found||!progress){locationMessage('Couldn’t reconnect to a mapped path. Follow your surroundings and return to a campus path.');$('#navigation-banner').hidden=true;return;}state.rerouteNotice=true;}
 if(state.follow){if(map.getZoom()<17)map.setZoom(17);map.panTo(latlng(state.gps),{animate:true});}
 if(offRouteSamples>0){speaker.stop();state.lastGuidance=null;$('#navigation-banner').hidden=false;$('#navigation-banner').innerHTML=icon('locate')+'<div><strong>Checking your route</strong><small>You may be off the mapped path. Waiting for another GPS reading.</small></div><button id="stop-live" class="icon-button" aria-label="Stop navigation">'+icon('close')+'</button>';$('#stop-live').onclick=()=>stopNavigation();return;}
 const guidance=currentGuidance(route,progress,c.accuracy);if(!guidance)return;
 state.lastGuidance=guidance.speech;
 const minutes=Math.max(1,Math.round(progress.remaining/75));
 const banner=$('#navigation-banner');banner.hidden=false;
 if(!$('#repeat-guidance')){
  banner.innerHTML='<span id="live-maneuver-icon" aria-hidden="true"></span><div class="navigation-copy"><strong id="live-instruction" aria-live="polite"></strong><small id="live-remaining"></small><div class="navigation-actions"><button id="repeat-guidance" class="button">'+icon('volume')+'Repeat</button><button id="mute-guidance" class="button"></button></div></div><button id="stop-live" class="icon-button" aria-label="Stop navigation">'+icon('close')+'</button>';
  $('#stop-live').onclick=()=>stopNavigation();$('#repeat-guidance').onclick=()=>speak(state.lastGuidance);
  $('#mute-guidance').onclick=()=>{state.voice=!state.voice;if(!state.voice)speaker.stop();updateMuteButton();if(state.voice)speak(state.lastGuidance);};
 }
 if($('#live-instruction').textContent!==guidance.title)$('#live-instruction').textContent=guidance.title;
 $('#live-maneuver-icon').innerHTML=icon(guidance.icon);$('#live-remaining').textContent=distanceText(progress.remaining)+' left · about '+minutes+' min';updateMuteButton();
 document.querySelectorAll('[data-step]').forEach(li=>{const active=+li.dataset.step===progress.stepIndex;li.classList.toggle('current-step',active);if(active)li.setAttribute('aria-current','step');else li.removeAttribute('aria-current');});
 if(state.voice){const cue=cueTracker.next(route,progress,c.accuracy);if(cue){speak((state.rerouteNotice?'Route updated. ':'')+cue);state.rerouteNotice=false;}}

 state.progress=progress.along;
}
function updateMuteButton(){const b=$('#mute-guidance');if(b){b.textContent=state.voice?'Mute voice':'Unmute voice';b.setAttribute('aria-pressed',String(!state.voice));}}
function locationError(error){
 ...
 speaker.stop();state.lastGuidance=null;locationMessage(messages[error.code]||'Couldn’t get your location.');
...
function stopNavigation(clearMessage=true){tracker.stop();state.navigating=false;locating=false;firstFix=false;offRouteSamples=0;cueTracker.reset();speaker.stop();state.lastGuidance=null;state.rerouteNotice=false;$('#navigation-banner').hidden=true;document.querySelectorAll('[data-step]').forEach(li=>{li.classList.remove('current-step');li.removeAttribute('aria-current');});if(clearMessage)locationMessage('');if($('#start-navigation'))$('#start-navigation').innerHTML=icon('navigate')+'Start walking';}
window.addEventListener('pagehide',()=>{tracker.stop();speaker.stop();});
setInterval(()=>{if(state.navigating&&lastFixTime&&Date.now()-lastFixTime>20000){waitingForGPS('GPS signal is stale. Waiting for a new location before giving turns.');}},5000);
